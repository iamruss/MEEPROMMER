...
#define VERSIONSTRING "MEEPROMMER $Revision: 1.4 $ $Date: 08/11/2016 14:00:00 $, CMD:R,r,w,W,V,u"
...
void readCommand() {
  ...
  do {//read 'til linebreak or buffer is full
    if (Serial.available()) {
      cmdbuf[idx++] = Serial.read();
    }
  } while (cmdbuf[idx - 1] != '\n' && idx < COMMANDSIZE);
  for (; idx < COMMANDSIZE; idx++) {
    cmdbuf[idx] = 0; //clear the rest of command buffer
  }
}
...
void loop() {
  ...
  switch (parseCommand()) {
    ...
    case WRITE_BIN:
      if (dataLength > BUFFERSIZE) {
        dataLength = BUFFERSIZE; //BUFFERSIZE is max
      }
      while (bytes < dataLength) {
        if (Serial.available()) {
          buffer[bytes++] = Serial.read();
