...
void fast_write(unsigned int address, byte data, bool fixed_delay)
{
  byte cyclecount=0;
...
void write_block(unsigned int address, byte* buffer, int len) {
  for (unsigned int i = 0; i < len; i++) {
    fast_write(address+i,buffer[i], false);
  }   
}
...
void loop() {
  ...
  case VERSION:
    ...
    break;
  case UNLOCK:
    fast_write(0x5555, 0xAA, true);
    fast_write(0x2AAA, 0x55, true);
    fast_write(0x5555, 0x80, true);
    fast_write(0x5555, 0xAA, true);
    fast_write(0x2AAA, 0x55, true);
    fast_write(0x5555, 0x20, true);
    Serial.println("%");
    break;
  default:
    break;    
